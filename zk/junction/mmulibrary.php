...
class MMULibrary {
	...
	public function connect($ip = null, $port = 44)
	{
		if($ip != null)
		{
			$this->ip = $ip;
	...
	public function getAttendance()
    {
        ...
        try {
            ...
            if (count($this->attendance_data) > 0) {
				...
				// while(strlen($attendance_data) > 40)
				// {
				// 	$us = unpack('H78', substr($attendance_data, 5, 42));
				// 	$sd[] = hexdec(substr($us[1], 56, 2));
                ...
                while (strlen($attendance_data) > 40) {
					...
					$us = unpack('H74', substr($attendance_data, 5, 39));
					// $u1s = hexdec(substr($us[1], 4, 2));
					// $u2s = hexdec(substr($us[1], 6, 2));
					// $uids = $u1s + ($u2s * 256);
					// $ids = str_replace("\0", '', hex2bin(substr($us[1], 8, 16)));
					$states = hexdec(substr($us[1], 56, 2));
					
	
                    $timestamp = $this->decodeTime(hexdec($this->reverseHex(substr($u[1], 58, 8))));
                    array_push($attendance, array($uid, $id, $states, $timestamp));
                    $attendance_data = substr($attendance_data, 40);
                }
